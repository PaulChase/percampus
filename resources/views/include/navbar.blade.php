...
$campuses = Cache::remember('campuses', Carbon::now()->addDay(), function () {
    return Campus::orderBy('name', 'asc')
  ...
  <nav id="header" class="w-full">
    <div
      ...
      <div class=" fixed   w-full h-full z-30 overflow-auto  hidden  top-0 left-0  "
        ...
        <div class="  order-3   w-4/6 md:w-2/6 h-full  rounded-r-lg bg-white text-base overflow-auto md:text-lg ">

          {{-- button to close menu --}}
          <button class=" right-3 top-3 absolute bg-gray-50 rounded-full px-2 py-1 focus:bg-gray-500 " id="closemenu">
            <i class="fa fa-times fa-2x text-gray-400"></i></button>
          @auth
          ...
          <ul class="grid grid-cols-1 gap-3  items-center  text-gray-700 pt-4">
            ...
            <li class="md:ml-2 hover:bg-green-100  p-2 focus:bg-green-100 bg-gray-100 md:bg-transparent">
              <a class="block font-semibold  no-underline hover:no-underline py-2  hover:text-black md:font-medium md:border-none md:p-0 "
                href="/dashboard"> <span class=""><i class="fa fa-user-circle mr-2"></i></span>My Profile
              </a>
            </li>
      ...
      <div class="order-2 md:order-3 lg:order-4 lg:col-span-1 flex items-center justify-center" id="nav-content">
        <button
          class="inline-block bg-green-500 px-3 py-2 rounded-md text-white text-sm font-bold md:text-base focus:bg-green-600 shadow-inner addPost ">Sell
          A...
        </button>
      ...
      <div
        class="order-3 lg:order-2 w-full  lg:col-span-5  mt-3 md:mt-2 lg:mt-0 @if (Request::is('allcampuses')) {{ 'hidden' }} @endif">
...
<div id="picktype" class=" fixed  w-full h-full z-30 overflow-auto  top-0 left-0 text-center  hidden"
  ...
  <div
    class=" bg-white bottom-0 absolute w-full rounded-t-lg p-4 lg:w-1/3 lg:h-2/3 lg:top-1/3 lg:right-0 lg:rounded-l-md">
    <button class=" float-right m-3 bg-gray-200 px-3 py-1 rounded-full focus:bg-gray-500"
      id="closepicktype">X</button><br>
    @if (Auth::check() and Auth::user()->role_id !== 1 and $postLeft <= 0)
      <div>
