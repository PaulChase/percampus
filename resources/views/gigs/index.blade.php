...
@section('title')
  Cheap Services offered by Students
@endsection
    ...
    <div class="grid lg:gap-4 grid-cols-1 gap-y-4 md:grid-cols-3 lg:grid-cols-4 py-3 max-w-7xl mx-auto w-full px-2">
      ...
      @if (count($posts) > 0)
        ...
        @foreach ($posts as $each_post)
          <div
            class="border-2 border-gray-200 md:border-none md:shadow-md grid  bg-white      rounded-md grid-cols-1  md:gap-y-2 ">
              ...
              <a
                ...
                @if (is_object($each_post->images()->first()))
                  <img src="{{ $each_post->images()->first()->Image_path }}"
                    class=" w-full  object-cover  rounded-t-md h-44 md:h-48   md:rounded-b-none md:rounded-t-sm"
                    lazy="loading" alt="">
                @endif
              </a>
            ...
            <div class="  flex flex-col justify-center md:justify-start px-3 py-2">
              ...
              <div class="py-2 flex items-center justify-between border-gray-100">
                ...
                @else
                  <div>
                    <img src="{{ $each_post->user->avatar }}" alt=""
                      class="inline-block rounded-full object-cover h-8 w-8">
                    @if ($each_post->alias)
                ...
                <div class="py-2 ">
                  ...
                  @else
                    <span class=" uppercase text-xs">from </span><span
                      class=" text-green-400 font-semibold ml-1 text-lg">N{{ $each_post->price }}</span>
                  @endif
