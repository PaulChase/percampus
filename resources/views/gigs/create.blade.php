    ...
    <div class="bg-gray-100 py-4">
        ...
        <div class="px-4 py-4 md:max-w-lg mx-auto md:shadow-lg rounded-md bg-white">
            ...
            <form method="POST" action="{{ route('gigs.store')}}" enctype="multipart/form-data" class=" space-y-4 text-gray-500">
                ...
                <div>
                    <label for="description" class="font-semibold" >Tell us more about the Service <b class=" text-red-500">*</b></label><br>
                    <textarea name="description" id="" cols="30" rows="7" class="  p-2 bg-gray-100 rounded-lg w-full mt-1  focus:outline-none focus:ring-2 focus:ring-green-200" placeholder="Provide more details about the Service such as experience level additional bonuses etc. This is your chance to convice the client to contact you" >{{ old('description')}}</textarea><br>
                    @error('description')
                        <small class="bg-red-300 p-2 inline-block rounded-sm text-sm mt-1">{{ $message }}</small>
                    @enderror
                </div>
                <div class="">
                    <label for="" class="font-semibold">Choose the Category of the Service<b class=" text-red-500">*</b> <br> <small>note you can't change this later</small></label>           
               ...
               @endif
                ...
                <div>
                    <label for="images" class="font-semibold"><i class="fa fa-photo "></i>Upload a maximum of 2 pics related to your Service (maybe your past projects, each image shouldn't be more than 2MB  server resources is cost now oo)</label>
                ...
                <div class="">
                    <input type="submit" name="submit" class="uppercase font-bold bg-green-500 hover:bg-green-600 focus:outline-none focus:bg-green-600 w-full rounded-md p-3  text-white  text-center" value="Submit Service">
                </div>
            </form>
        </div>
    </div>
