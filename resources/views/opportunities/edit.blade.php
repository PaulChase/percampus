        ...
        <div class="px-4 py-4 md:max-w-lg mx-auto md:shadow-lg rounded-md bg-white">
            ...
            <form method="POST" action="{{ route('opportunities.update', ['opportunity' => $post->id])}}" enctype="multipart/form-data" class=" space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" id="" cols="30" rows="8" class="  p-2 bg-gray-100 rounded-lg w-full mt-1  focus:outline-none focus:ring-2 focus:ring-green-200" placeholder="give accurate information concerning the opportunity such as who is eligible and the necessary requirements to apply etc">{{ $post->description}}</textarea><br>
                    @error('description')
